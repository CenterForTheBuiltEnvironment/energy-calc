...
class EnergyCalcModel:
    ...
    __climates__ = ['Miami', 'Phoenix', 'Fresno', 
        'San Francisco', 'Baltimore', 'Chicago', 'Duluth']
    ...
    def calculate(self, sp0, sp1, climate_zone, vav_type, vintage, vav_fixed, cool_side, verbose=False):
        ...
        if cool_side:
            # cooling side
            # new method: truncate
            if sp0 < 22.225: sp0 = 22.225
            if sp1 < 22.225: sp1 = 22.225
            if sp0 > 30: sp0 = 30
            if sp1 > 30: sp1 = 30
        else:
            # heating side
            if sp0 > 21.1: sp0 = 21.1
            if sp1 > 21.1: sp1 = 21.1
            if sp0 < 17.6: sp0 = 17.6
            if sp1 < 17.6: sp1 = 17.6
    ...
    def savings(self, sp0, sp1, verbose=False, component_savings=False):
        ...
        terminal_heating_savings_per = 100 * (terminal_heating0 - terminal_heating1) / hvac_0
        central_heating_savings_per = 100 * (central_heating0 - central_heating1) / hvac_0
        ...
        if component_savings:
            ...
            rv['component_savings']['terminal_heating_component_savings_per'] = 100 * (terminal_heating0 - terminal_heating1) / terminal_heating0
            rv['component_savings']['central_heating_component_savings_per'] = 100 * (central_heating0 - central_heating1) / central_heating0
            rv['component_savings']['cooling_component_savings_per'] = 100 * (cooling_0 - cooling_1) / cooling_0
            rv['component_savings']['fan_component_savings_per'] = 100 * (fans_0 - fans_1) / fans_0

        for k in rv['chart_data']:
          if isnan(rv['chart_data'][k]): rv['chart_data'][k] = 0.
          if rv['chart_data'][k] < 0: rv['chart_data'][k] = 0.

        for k in rv['table_data']:
          if isnan(rv['table_data'][k]): rv['table_data'][k] = 0.
          if rv['table_data'][k] < 0: rv['table_data'][k] = 0.
